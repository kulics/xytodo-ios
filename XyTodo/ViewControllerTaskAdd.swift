...
class ViewControllerTaskAdd: UITableViewController, UITextFieldDelegate, UIPopoverPresentationControllerDelegate
{
    ...
    {
        super.viewDidLoad()
        self.navigationItem.title = NSLocalizedString("page_title_task_add",comment: "")
        mActionButton.title = NSLocalizedString("button_save",comment: "")
        //获取本地数据
        mApp = UIApplication.shared.delegate as? AppDelegate
        mDataTask = ModelTask()
        mDataTask.note = ""
        mDataTask.color = "blue"
        //从设置中获取最后使用的颜色
        if let last = UserDefaults.standard.value(forKey: "color_last") as? String {
            mDataTask.color = last
        }
        //获取时间
        let time = Int(Date().timeIntervalSince1970)
        mDataTask.timeCreate = time
        mDataTask.timeTarget = mDataTask.timeCreate
        mDataTask.timeDone = 0
        mDataTask.timeSort = mDataTask.timeCreate
        mDataTask.status = 0
        //隐藏空白项
    ...
    @IBAction func actionPalette(_ sender : Any)
    {
        ...
        vc.addAction { (color) in
            ...
            //将最后使用的颜色保存到配置
            UserDefaults.standard.set(color, forKey: "color_last")
    ...
    @IBAction func actionSave(_ sender : Any)
    {
        if mDataTask.content.lengthOfBytes(using: .utf8) > 0 {
            _ = mApp.DBMGet().TaskAdd(model: mDataTask)
